...
echo "merging merged_exons and introns"
...
then
	...
	bedtools complement -i annotation_gene_for.bed -g annotation_chr.genome | awk '{print $1,$2,$3,"intergenic",".","+"}' | sed -e 's/ /\t/g' | sort -k1,1 -k2,2n -k3,3n | uniq > intergenics_for.bed
	bedtools complement -i annotation_gene_rev.bed -g annotation_chr.genome | awk '{print $1,$2,$3,"intergenic",".","-"}' | sed -e 's/ /\t/g' | sort -k1,1 -k2,2n -k3,3n | uniq > intergenics_rev.bed
fi


# merge of "exon_intron_sorted.bed" and "intergenics.bed"
echo "merge of exon_intron_sorted.bed and intergenics.bed"
...
# strand 1:
echo "retrieve backsplice regions"
...
# strand 2:
echo "retrieve backsplice regions (strand 2)"
...
echo "retrieve the regions intersected by the backsplice, for each backsplice (strand 1)"
# strand 1:
...
# strand 2:
echo "retrieve the regions intersected by the backsplice, for each backsplice (strand 2)"
...
# strand 1:
echo "each intersected region put on a different row, merging adjacent or overlapping fragments (strand 1)"
...
# strand 2:
echo "each intersected region put on a different row, merging adjacent or overlapping fragments (strand 2)"
...
# strand 1:
echo "each backsplice with intersected regions put on a different row (strand 1)"
...
# strand 2:
echo "each backsplice with intersected regions put on a different row (strand 2)"
...
# strand 1:
echo "extract the sequence of each region (strand 1)"
...
# strand 2:
echo "extract the sequence of each region (strand 2)"
...
# strand 1:
echo "retrieve the sequence of each backsplice (strand 1)"
if [ $N1_plus -gt 0 ]
then
...
if [ $N1_plus -gt 0 ] || [ $N1_minus -gt 0 ]
then
	#doesn't work for sequences > 150000 nt
	cat backsplice_sequence_bed_1.txt | sed -r 's/^([0-9A-Z]*):([0-9]*)-([0-9]*)\t([A-Z]*)/echo "\1:$((\2+1))-\3\t\4"/e' > backsplice_sequence_1.txt
	cat backsplice_sequence_1.txt | sed -e 's/^/>/; s/\t/\n/' > backsplice_sequence_1.fa
fi


# strand 2:
echo "retrieve the sequence of each backsplice (strand 2)"
...
echo "retrieve circRNA length (strand 1)"
if [ $N1_plus -gt 0 ] || [ $N1_minus -gt 0 ]
then
...
echo "retrieve circRNA length (strand 2)"
if [ $N2_plus -gt 0 ] || [ $N2_minus -gt 0 ]						
then
